...
public class Ex01 
{
	...
	// 		있는 것(설계 도면, 거푸집 등으로 표현)
	// 		**파일 형태로 되어 있으면 클래스를 객체로 만들어야만 사용사능.
	...
	//			  클래스, 멤버필드, 멤버메소드,생성자 모두 맨 앞에 표시할수 있다.(생략가능)
	//	1)public : 누구나 다 접근 가능
	//	2)protected : 같은 패키지와 상속 관계에서는 접근 가능
	//	3) 생략 : 같은 패키지에서는 접근 가능
	//  4) private : 외부에서는 무조건 접근 불가
	//				내부멤버(필드,메소드)끼리만 접근 가능(아무도 접근 못함.)
	// - 클래스 종류 : 일반적인 클래스들은 이 부분을 생략
	//  		   특정 클래스는 해당 예약어를 사용 (final, abstract)
	// - class : 클래스임을 나타내는 예약어
	// - 클래스이름 : 클래스이름 = 저장이름 = 생성자
	//			  첫글자에 대문자 나머지 소문자,두단어 이상일때 단어들의 첫글자는 대문자
	// 			  특수문자 사용불가, 숫자는 중간이나 끝에 사용가능, 띄어쓰기 불가
	...
	// 			모든 글자가 대문자, 단어와 단어 사이에 _(언더바, 언더스코어)사용가능.
	...
	// 			  main() 는 JVM이 호출한다.(main()가 끝나면 JVM으로 되돌아감.)
	//			  JVM으로 되돌아 가면 프로그램이 종료됨.
	// -메소드 구성 : [접근제한자] [메소드종류] 반환형 메소드이름([인자,매개변수]){실행내용;}
	// 1) 접근제한자 : 클래스 접근제한자와 같음.
	//			  public > protected > 생략 > private
	// 2) 메소드종류 : instance 메소드와 static 메소트로 나뉨.
	//				instance 메소드가 일반 메소드로 이 부분을 생략한다.
	//				static 메소드는 반드시 static 예약어를 사용한다.
	// ** 3) 반환형 : 메소드는 자기를 호출한 곳으로 되돌아 간다.
	// 				이 때 실행한 결과를 가지고 갈 때 자료형을 반환형이라고 함.
	//				만약 실행한 결과를 가지고 가지 않을 때는 반환형에 void라고 쓴다.
	//				ex) public static void main(){}
	//				- 누구나 다 접근 가능하고, static메소드이고, 되돌아 갈때 무언가를 가지고 가지않음.
	// 4) 인자 = 매개변수 : 메소드가 동작을 할때 필요한 정보를 외부에서 받아서 사용할 때 인자가 필요하다.
	//	오버로딩 : 같은 클래스 안에서 메소드이름은 같고, 인자의 갯수나 인자의 자료형이 다른 메소드를 말한다.
	// 5) getter() / setter()
	//		getter() : 호출하는 입장에서 메소드를 이용하여 멤버필드의 데이터를 얻어낼 수 있다.
	//		setter() : 호출하는 입잡에서 메소드를 이용하여 멤버필드의 데이터를 설정할 수 있다.
	
	// 4. 생성자 : 클래스를 개체로 만들때 한 번 호출되는 것
	//	- 목적 : 멤버필드(번수, 상수)의 초기값을 설정하는 것이 목적임.
	...
	//	- 만들어지 객체 안에 존재하는 멤버필드나 멤버메소드를 사용하기 위한 방법
